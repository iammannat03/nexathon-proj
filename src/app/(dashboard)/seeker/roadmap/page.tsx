...
const roadmap: Record<string, RoadmapData> = {
  "backend development": {
    ...
    recommendedResources: ["Node.js Documentation", "Express.js Guide"],
    practicalProjects: ["Build a REST API", "Create a CRUD application"],
  },
...
const RoadmapPage = () => {
  ...
  const [roadMap, setRoadMap] = useState<RoadmapData | null>(null); // State to hold roadmap data
  ...
  useEffect(() => {
    ...
    const fetchJobData = async () => {
      if (jobIdFromQuery) {
        const fetchedJob = await appwriteService.getDoc(jobIdFromQuery); // Fetch job data
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <Card className="p-6 shadow-lg">
        <h2 className="mb-4 text-2xl font-semibold">Roadmap for {job.key}</h2>
        <div className="mb-6">
          <h3 className="text-xl font-semibold">Subtopics:</h3>
